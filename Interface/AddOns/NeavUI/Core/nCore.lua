...
N.GetHealthText = function(unit, cur, max)
    local uconf = C['unitframes'].units[N.cUnit(unit)]
...
N.PostUpdateIcon = function(icons, unit, icon, index, offset)
    ...
    if (C['unitframes'].units.target.colorPlayerDebuffsOnly) then
        if (unit == 'target') then 
            if (icon.debuff) then
    ...
    if (icon.remaining) then
        if (unit == 'target' and icon.debuff and not IsMine(icon.owner) and (not UnitIsFriend('player', unit) and UnitCanAttack(unit, 'player') and not UnitPlayerControlled(unit)) and not C['unitframes'].units.target.showAllTimers ) then
...
N.UpdateAuraIcons = function(auras, button)
    if (not button.Shadow) then
        ...
        if (C['unitframes'].show.disableCooldown) then
        ...
        if (not button.Shadow) then
            ...
            button.Shadow:SetTexture('Interface\\AddOns\\NeavUI\\Media\\borderBackground')
