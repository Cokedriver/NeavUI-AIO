...
local function GetUnitPVPIcon(unit) 
    ...
    if (UnitIsPVPFreeForAll(unit)) then
        if (C['tooltip'].showPVPIcons) then
            return '|TInterface\\AddOns\\NeavUI\\Media\\UI-PVP-FFA:12|t'
        else
            return '|cffFF0000# |r'
        end
    elseif (factionGroup and UnitIsPVP(unit)) then
        if (C['tooltip'].showPVPIcons) then
            return '|TInterface\\AddOns\\NeavUI\\Media\\UI-PVP-'..factionGroup..':12|t'
        else
            return '|cff00FF00# |r'
        end
    else
...
GameTooltip:HookScript('OnTooltipSetUnit', function(self, ...)
    ...
    if (UnitExists(unit) and UnitName(unit) ~= UNKNOWN) then
        ...
        if (C['tooltip'].healthbar.reactionColoring or C['tooltip'].healthbar.customColor.apply) then
