...
local function CreateIndicators(self, unit)
    ...
    if (buffs) then
        for key, spell in pairs(buffs) do
            ...
            icon:SetWidth(C['raidframes'].units.raid.indicatorSize)
            icon:SetHeight(C['raidframes'].units.raid.indicatorSize)
...
local function GetHealthText(unit, cur, max)
    ...
    if (UnitIsDeadOrGhost(unit) or not UnitIsConnected(unit)) then
        ...
        if ((cur/max) < C['raidframes'].units.raid.deficitThreshold) then
...
local function CreateRaidLayout(self, unit)
    ...
    if (C['raidframes'].units.raid.horizontalHealthBars) then
    ...
    if (C['raidframes'].units.raid.showMainTankIcon) then
    ...
    self.FreebAuras:SetSize(C['raidframes'].units.raid.iconSize, C['raidframes'].units.raid.iconSize)
    ...
    if (C['raidframes'].units.raid.showTargetBorder) then
...
f.t:SetText('oUF_Neav Raid_Anchor "'..C['raidframes'].units.raid.layout.initialAnchor..'"')
...
oUF:Factory(function(self)
    ...
    local rlayout = C['raidframes'].units.raid.layout
    ...
    for i = 1, rlayout.numGroups do
        table.insert(raid, self:SpawnHeader('oUF_Neav_Raid'..i, nil, 'solo,party,raid',
            ...
            self:SetWidth(%d)
            self:SetHeight(%d)
            ]]):format(C['raidframes'].units.raid.width, C['raidframes'].units.raid.height),

            'showRaid', true,
            'showParty', C['raidframes'].units.raid.showParty,
            'showPlayer', true,
            'showSolo', C['raidframes'].units.raid.showSolo,
        ...
        raid[i]:SetScale(C['raidframes'].units.raid.scale) 
    end
end)