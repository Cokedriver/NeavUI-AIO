...
if C['plates'].enable ~= true then return end
...
local glowTexture = 'Interface\\AddOns\\NeavUI\\Media\\NameplateNewGlow'
local overlayTexture = 'Interface\\AddOns\\NeavUI\\Media\\NameplateOverlay'
local whiteOverlay = 'Interface\\AddOns\\NeavUI\\Media\\NameplateIconOverlay'
...
local function UpdateThreatColor(self)
    ...
    if (lowThreat and not self.Glow:IsVisible() and isEnemy and C['plates'].enableTankMode) then
        ...
        if (C['plates'].colorNameWithThreat) then
...
local function UpdateHealthText(self)
    ...
    if (perc >= 100 and currentValue > 5 and C['plates'].showFullHP) then
...
local function UpdatePlate(self)
    ...
    if (C['plates'].showEliteBorder) then
    ...
    if (C['plates'].showTotemIcon) then
...
local function SkinPlate(self)
    ...
    f:HookScript('OnUpdate', function(_, elapsed)
        ...
        if (f.elapsed >= 0.1) then
            ...
            if (C['plates'].showTargetBorder) then
