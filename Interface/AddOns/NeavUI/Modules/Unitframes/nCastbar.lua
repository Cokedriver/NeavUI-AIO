...
function N.CreateCastbars(self, unit)
    local config = C['unitframes'].units[N.cUnit(unit)].castbar
    ...
    if (N.MultiCheck(unit, 'player', 'target', 'focus', 'pet') and config and config.show) then 
        ...
        self.Castbar.PostCastStart = function(self, unit)
            ...
            if (C['unitframes'].units.pet.castbar.ignoreSpells) then
                if (unit == 'pet') then
                    self:SetAlpha(1)

                    for _, spellID in pairs(C['unitframes'].units.pet.castbar.ignoreList) do
