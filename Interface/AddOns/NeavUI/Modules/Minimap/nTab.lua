...
if (not C['minimap'].tab.show) then
    return
end
...
local performanceIcon = '|TInterface\\AddOns\\NeavUI\\Media\\MinimapPerformance:10|t'
...
f:SetHeight(30)
f:SetAlpha(C['minimap'].tab.alphaNoMouseover)
...
local function HideTab()
    ...
    if (C['minimap'].tab.showAlways) then
        return
    end
    ...
    securecall('UIFrameFadeOut', f, 0.15, f:GetAlpha(), C['minimap'].tab.alphaNoMouseover)
end
...
local function ShowTab()
    ...
    securecall('UIFrameFadeIn', f, 0.15, f:GetAlpha(), C['minimap'].tab.alphaMouseover)
end

if (C['minimap'].tab.showBelowMinimap) then
...
local function SlideUp()
    f.mod = 1
    f.limit = C['minimap'].tab.showBelowMinimap and -6 or 21
    f.speed = 200
...
local function SlideDown()
    f.mod = -1
    f.limit = C['minimap'].tab.showBelowMinimap and -21 or 6
    f.speed = -200
    f:SetScript('OnUpdate', SlideFrame)
end
...
f:SetScript('OnEnter', function()
    if (not C['minimap'].tab.showAlways) then
        ShowTab()
        if (C['minimap'].tab.showBelowMinimap) then
            SlideDown()
        else
