...
if (not C['nTooltip'].healthbar.showHealthValue) then
    return
end
...
bar.Text:SetPoint('CENTER', bar, C['nTooltip'].healthbar.textPos, 0, 1)

if (C['nTooltip'].healthbar.showOutline) then
    bar.Text:SetFont(C['nMedia'].font, C['nTooltip'].healthbar.fontSize, 'THINOUTLINE')
    bar.Text:SetShadowOffset(0, 0)
else
    bar.Text:SetFont(C['nMedia'].font, C['nTooltip'].healthbar.fontSize)
...
GameTooltipStatusBar:HookScript('OnValueChanged', function(self, value)
    ...
    local fullString = GetHealthTag(C['nTooltip'].healthbar.healthFullFormat, value, max)
    local normalString = GetHealthTag(C['nTooltip'].healthbar.healthFormat, value, max)
