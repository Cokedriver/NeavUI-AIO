...
if C['nPower'].enable ~= true then return end
...
f:SetScale(1.4)
f:SetSize(18, 18)
f:SetPoint(C['nPower'].position.selfAnchor, C['nPower'].position.frameParent, C['nPower'].position.offSetX, C['nPower'].position.offSetY)
f:EnableMouse(false)
...
if (playerClass == 'WARLOCK' and C['nPower'].showSoulshards or playerClass == 'PALADIN' and C['nPower'].showHolypower or playerClass == 'PRIEST' and C['nPower'].showShadowOrbs) then
    ...
    if (C['nPower'].extraFontOutline) then
        ...
        f.extraPoints:SetFont(C['nMedia'].font, C['nPower'].extraFontSize)
...
if (playerClass == 'DEATHKNIGHT' and C['nPower'].rune.showRuneCooldown) then
    ...
    for i = 1, 6 do
        ...
        if (C['nPower'].rune.runeFontOutline) then
            f.Rune[i]:SetFont(C['nMedia'].font, C['nPower'].rune.runeFontSize, 'THINOUTLINE')
            f.Rune[i]:SetShadowOffset(0, 0)
        else
            f.Rune[i]:SetFont(C['nMedia'].font, C['nPower'].rune.runeFontSize)
...
f.Power:SetSize(C['nPower'].sizeWidth, 3)
f.Power:SetPoint('CENTER', f, 0, -23)
f.Power:SetStatusBarTexture('Interface\\AddOns\\NeavUI\\nMedia\\nTextures\\PowerbarTexture')
f.Power:SetAlpha(0)

f.Power.Value = f.Power:CreateFontString(nil, 'ARTWORK')

if (C['nPower'].valueFontOutline) then
    f.Power.Value:SetFont(C['nMedia'].font, C['nPower'].valueFontSize, 'THINOUTLINE')
    f.Power.Value:SetShadowOffset(0, 0)
else
    f.Power.Value:SetFont(C['nMedia'].font, C['nPower'].valueFontSize)
    f.Power.Value:SetShadowOffset(1, -1)
end

f.Power.Value:SetPoint('CENTER', f.Power, 0, C['nPower'].valueFontAdjustmentX)
...
f.Power.Background:SetTexture('Interface\\AddOns\\NeavUI\\nMedia\\nTextures\\PowerbarTexture')
...
f:SetScript('OnEvent', function(self, event, arg1)
    ...
    if (event == 'RUNE_TYPE_UPDATE' and C['nPower'].rune.showRuneCooldown) then
