...
if (not C['nMinimap'].tab.show) then
    return
end
...
local performanceIcon = '|TInterface\\AddOns\\NeavUI\\nMedia\\nTextures\\MinimapPerformance:10|t'
...
f.Left.Text:SetFont(C['nMedia'].font, 12)
...
f.Right.Text:SetFont(C['nMedia'].font, 12)
...
f.Center.Text:SetFont(C['nMedia'].font, 12)
...
local function HideTab()
    ...
    if (C['nMinimap'].tab.showAlways) then
        return
    end
    ...
    securecall('UIFrameFadeOut', f, 0.15, f:GetAlpha(), C['nMinimap'].tab.alphaNoMouseover)
end
...
local function ShowTab()
    ...
    securecall('UIFrameFadeIn', f, 0.15, f:GetAlpha(), C['nMinimap'].tab.alphaMouseover)
end

if (C['nMinimap'].tab.showBelowMinimap) then
...
local function SlideUp()
    f.mod = 1
    f.limit = C['nMinimap'].tab.showBelowMinimap and -6 or 21
    f.speed = 200
...
local function SlideDown()
    f.mod = -1
    f.limit = C['nMinimap'].tab.showBelowMinimap and -21 or 6
    f.speed = -200
    f:SetScript('OnUpdate', SlideFrame)
end
...
f:SetScript('OnEnter', function()
    if (not C['nMinimap'].tab.showAlways) then
        ShowTab()
        if (C['nMinimap'].tab.showBelowMinimap) then
            SlideDown()
        else
