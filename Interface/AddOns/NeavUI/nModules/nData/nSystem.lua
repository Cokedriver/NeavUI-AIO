...
if C['nData'].system and C['nData'].system > 0 then
	...
	Stat:SetScript("OnEnter", function(self)
		...
		GameTooltip:AddLine(hexa..N.myname.."'s"..hexb.." Latency")
		GameTooltip:AddLine' '			
		if C['nData'].fps.enable == true then 
			if C['nData'].fps.home == true then
				GameTooltip:AddDoubleLine("Home Latency: ", string.format(homeLatencyString, latencyHome), 0.80, 0.31, 0.31,0.84, 0.75, 0.65)
			elseif C['nData'].fps.world == true then
				GameTooltip:AddDoubleLine("World Latency: ", string.format(worldLatencyString, latencyWorld), 0.80, 0.31, 0.31,0.84, 0.75, 0.65)
			elseif C['nData'].fps.both == true then
				GameTooltip:AddDoubleLine("Home Latency: ", string.format(homeLatencyString, latencyHome), 0.80, 0.31, 0.31,0.84, 0.75, 0.65)
				GameTooltip:AddDoubleLine("World Latency: ", string.format(worldLatencyString, latencyWorld), 0.80, 0.31, 0.31,0.84, 0.75, 0.65)
			end
		end
		if bandwidth ~= 0 then
			GameTooltip:AddDoubleLine(L.Data_bandwidth , string.format(bandwidthString, bandwidth),0.69, 0.31, 0.31,0.84, 0.75, 0.65)
