...
local function CreateIndicators(self, unit)
    ...
    if (buffs) then
        for key, spell in pairs(buffs) do
            ...
            icon:SetWidth(C['nRaidframes'].units.raid.indicatorSize)
            icon:SetHeight(C['nRaidframes'].units.raid.indicatorSize)
            ...
            icon.icon:SetTexture('Interface\\AddOns\\NeavUI\\nMedia\\nTextures\\borderIndicator')
...
local function UpdatePower(self, _, unit)
    ...
    if (powerToken == 'MANA' and UnitHasMana(unit)) then
        if (not self.Power:IsVisible()) then
            ...
            if (C['nRaidframes'].units.raid.manabar.horizontalOrientation) then
...
local function GetHealthText(unit, cur, max)
    ...
    if (UnitIsDeadOrGhost(unit) or not UnitIsConnected(unit)) then
        ...
        if ((cur/max) < C['nRaidframes'].units.raid.deficitThreshold) then
...
local function CreateRaidLayout(self, unit)
    ...
    self.Health:SetStatusBarTexture('Interface\\AddOns\\NeavUI\\nMedia\\nTextures\\statusbarTexture', 'ARTWORK')
    self.Health:SetAllPoints(self)
    self.Health:SetOrientation(C['nRaidframes'].units.raid.horizontalHealthBars and 'HORIZONTAL' or 'VERTICAL')	
    ...
    if (C['nRaidframes'].units.raid.smoothUpdates) then
    ...
    --self.Health.bg:SetTexture('Interface\\AddOns\\NeavUI\\nMedia\\nTextures\\statusbarTexture')
    ...
    self.Health.Value:SetFont(font, C['nRaidframes'].font.fontSize)
    ...
    self.Name:SetFont(font, C['nRaidframes'].font.fontBigSize)
    ...
    if (C['nRaidframes'].units.raid.manabar.show) then
    ...
    if (C['nRaidframes'].units.raid.horizontalHealthBars) then
    ...
    if (C['nRaidframes'].units.raid.showMouseoverHighlight) then    
        self.Mouseover = self.Health:CreateTexture(nil, 'OVERLAY')
        self.Mouseover:SetAllPoints(self.Health)
        self.Mouseover:SetTexture('Interface\\AddOns\\NeavUI\\nMedia\\nTextures\\statusbarTexture')
    ...
    self.ThreatGlow:SetBackdrop({edgeFile = 'Interface\\AddOns\\NeavUI\\nMedia\\nTextures\\textureGlow', edgeSize = 3})
    self.ThreatGlow:SetBackdropBorderColor(0, 0, 0, 0)
    ...
    if (C['nRaidframes'].units.raid.showMainTankIcon) then
    ...
    self.FreebAuras:SetSize(C['nRaidframes'].units.raid.iconSize, C['nRaidframes'].units.raid.iconSize)
...
f:SetSize(80, 80)
f:SetPoint('TOPLEFT', UIParent, 25, -200)
...
f.t:SetAllPoints(f)
f.t:SetFont(font, 13)
f.t:SetText('oUF_Neav Raid_Anchor "'..C['nRaidframes'].units.raid.layout.initialAnchor..'"')
...
SlashCmdList['oUF_Neav_Raid_AnchorToggle'] = function()
    if (InCombatLockdown()) then
        f:Hide()
        print('NeavUI: You cant do this in combat!')
        return
    end
...
oUF:Factory(function(self)
    ...
    local rlayout = C['nRaidframes'].units.raid.layout
    ...
    for i = 1, rlayout.numGroups do
        table.insert(raid, self:SpawnHeader('oUF_Neav_Raid'..i, nil, 'solo,party,raid',
            ...
            self:SetWidth(%d)
            self:SetHeight(%d)
            ]]):format(C['nRaidframes'].units.raid.width, C['nRaidframes'].units.raid.height),

            'showRaid', true,
            'showParty', C['nRaidframes'].units.raid.showParty,
            'showPlayer', true,
            'showSolo', C['nRaidframes'].units.raid.showSolo,
        ...
        raid[i]:SetScale(C['nRaidframes'].units.raid.scale) 
    end
end)